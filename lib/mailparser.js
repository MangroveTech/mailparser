    ...
    for(i=0, len=lines.length; i < len; i++){
        ...
        if(this._state == STATES.body){
            ...
            if(this._processStateBody(line) === true){
                this._added = false;
                if (this._currentNode.meta.partId === '1' && !this._added) { 
                    this._added = true;  
                    this._parts.push({
                        meta: this._currentNode.meta,
                        childNodes: this._currentNode.childNodes
                    });
                }
                continue;
            }
...
MailParser.prototype._processStateHeader = function(line){
    ...
    if (!line.length || (line.length >= 76 && line.indexOf('\:') === -1)) {
        if(lastPos>=0){
        ...
        if(textContent && (!this._currentNode.meta.contentDisposition || this._currentNode.meta.contentDisposition == "inline")){
            ...
            this._currentNode.meta.attachment = false;
          ...
          !this._currentNode.meta.mimeMultipart){
            ...
            this._currentNode.meta.attachment = true;
        }else if(!this._currentNode.meta.contentDisposition && this._currentNode.meta.contentId && this._currentNode.meta.transferEncoding == "base64") {
            this._currentNode.attachment = true;
            this._currentNode.meta.attachment = true;
    ...
    if(line.substr(0, 2) == "--"){
        for(i=0, len = this._multipartTree.length; i<len; i++){
            ...
            if(line == "--" + this._multipartTree[i].boundary){
                ...
                //add Part for content.
                var nodeParentPartId = this._multipartTree[i].node.meta.partId;
                var nodeParentChildCnt = this._multipartTree[i].node.childNodes.length;
                node.meta.partId = nodeParentPartId + '.' + nodeParentChildCnt;
...
MailParser.prototype._processHeaderLine = function(pos){
    ...
    switch(key){
        ...
        case "bcc":
            ...
            break;
        case "sender":
            if(this._currentNode.sender && this._currentNode.sender.length){
                this._currentNode.sender = this._currentNode.sender.concat(mimelib.parseAddresses(value));
            }else{
                this._currentNode.sender = mimelib.parseAddresses(value);
            }
            break;
        case "x-priority":
...
MailParser.prototype._getReturnValue = function() {
    var returnValue = {}, i, len;

    this.mailData = {html:[], text:[], calendar:[], attachments:[], contents: []};
...
MailParser.prototype._getWaveReturnValue = function () {
    var returnValue = {contents:[]}, i, len;
    var tmpContent;

    returnValue.headers = this.mimeTree.parsedHeaders;

    if(this.mimeTree.subject){
        returnValue.headers.subject = this.mimeTree.subject;
    }

    if(this.mimeTree.references){
        returnValue.headers.references = this.mimeTree.references;
    }

    if(this.mimeTree.messageId){
        returnValue.headers['message-id'] = this.mimeTree.messageId;
    }

    if(this.mimeTree.inReplyTo){
        returnValue.headers['in-reply-to'] = this.mimeTree.inReplyTo;
    }

    // if(this.mimeTree.priority){
    //     returnValue.headers.priority = this.mimeTree.priority;
    // }

    if(this.mimeTree.from){
        returnValue.headers.from = this.mimeTree.from;
    }

    if (this.mimeTree.sender) {
        returnValue.headers.sender = this.mimeTree.sender;
    }

    if(this.mimeTree.replyTo){
        returnValue.headers['reply-to'] = this.mimeTree.replyTo;
    }

    if(this.mimeTree.to){
        returnValue.headers.to = this.mimeTree.to;
    }

    if(this.mimeTree.cc){
        returnValue.headers.cc = this.mimeTree.cc;
    }

    if(this.mimeTree.bcc){
        returnValue.headers.bcc = this.mimeTree.bcc;
    }

    if(this.mimeTree.meta.date){
        returnValue.headers.date = this.mimeTree.meta.date.getTime();
    }

    if (this.mailData.contents.length) {
        for(i=0, len=this.mailData.contents.length; i<len; i++){
            tmpContent = this.mailData.contents[i].content;
            if (tmpContent.contentDisposition) {
                tmpContent.fileName = tmpContent.generatedFileName;
                // delete tmpContent.fileName;
                delete tmpContent.generatedFileName;
            } else {
                delete tmpContent.transferEncoding;
            }
            delete tmpContent.messageId;
            delete tmpContent.date;

            returnValue.contents.push(this.mailData.contents[i].content);
        }
    }
...
MailParser.prototype._processMimeTree = function(){
    ...
    if (this.options.wave) {
        returnValue = this._getWaveReturnValue();
    } else {
        returnValue = this._getReturnValue();
    }
    ...
    if(!node.attachment){
        switch(node.meta.contentType){
            case "text/html":
                ...
                if(node.content){
                    node.meta.content = this._updateHTMLCharset(node.content || "");
                }
                this.mailData.contents.push({content: node.meta || {}, level: level});
